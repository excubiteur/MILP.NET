...
namespace Examples
{
    class Diet
    {
        public static void Run()
        {
            ...
            var NUTR = m.CreateSet().Add(new string[] {
                "A", "B1", "B2", "C" });

            var FOOD = m.CreateSet().Add(new string[] {
                "BEEF", "CHK", "FISH", "HAM", "MCH", "MTL", "SPG", "TUR" });

            var cost = m.CreateParam(FOOD).Add(new double[] {
                3.19, 2.59, 2.29, 2.89, 1.89, 1.99, 1.99, 2.49 });

            var f_min = m.CreateParam(FOOD).Add(new double[] {
                0,0,0,0,0,0,0,0 });

            var f_max = m.CreateParam(FOOD).Add(new double[] {
                100,100,100,100,100,100,100,100 });

            var n_min = m.CreateParam(NUTR).Add(new double[] {
                700,700,700,700 });

            var n_max = m.CreateParam(NUTR).Add(new double[] {
                10000,10000,10000,10000 });

            var amt = m.CreateParam(NUTR, FOOD).Add(new double[,] {
